...
import { askGemini, extractImportantPoints, generateMailReply, suggestMailSubject, summarizeMail } from "../services/aiService.js";


export const generateGeminiResponse = expressAsyncHandler(
    async (req, res) => {
...
export const summarizeEmail = expressAsyncHandler(async (req, res) => {
    ...
    const summary = await summarizeMail(
        subject.trim(),
        message.trim()
    );
...
export const generateReply = expressAsyncHandler(async (req, res) => {
    ...
    const reply = await generateMailReply(
        subject.trim(),
        message.trim()
    );
